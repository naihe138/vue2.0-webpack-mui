    ...
    $.fn.off = function (event, selector, callback) {
      return this.each(function () {
        ...
        if (delegates[_mid]) {
          ...
          if ((
            !delegates[_mid][event] || $.isEmptyObject(delegates[_mid][event]))) {
...
(
  function (document) {
    if (!(
      "classList" in document.documentElement) && Object.defineProperty && typeof HTMLElement !== 'undefined') {
  ...
  function (window) {
    if (!window.requestAnimationFrame) {
      ...
      window.requestAnimationFrame = window.webkitRequestAnimationFrame || function (callback, element) {
          ...
          var timeToCall = Math.max(0, 16.7 - (
                                       currTime - lastTime));
      ...
      document.addEventListener('focusin', function (e) {
        ...
        if (target.tagName && (
          ...
          for (; target && target !== document; target = target.parentNode) {
            ...
            if (classList && classList.contains(CLASS_BAR_TAB) || classList.contains(CLASS_BAR_FOOTER) ||
                classList.contains(CLASS_BAR_FOOTER_SECONDARY) || classList.contains(CLASS_BAR_FOOTER_SECONDARY_TAB)) {
              isFooter = true;
              break;
            }
          }
...
(
  function ($, window) {
    ...
    var calDelta = function (touch) {
      ...
      touch.deltaX = prevDelta.x + (
        center.x - offset.x);
      touch.deltaY = prevDelta.y + (
        center.y - offset.y);
    };
    ...
    var getTouches = function (event, touch) {
      ...
      if ((
          type === $.EVENT_START || type === $.EVENT_MOVE) && allTouches.length === 1) {
  ...
  function ($, name) {
    ...
    var handle = function (event, touch) {
      ...
      switch (event.type) {
        ...
        case $.EVENT_CANCEL:
          touch.flick = false;
          if (session.flickStart && options.flickMaxTime > (
            now - flickStartTime) && touch.distance > options.flickMinDistince) {
            touch.flick = true;
      ...
      switch (event.type) {
        case $.EVENT_END:
          ...
          if (touch.distance < options.tapMaxDistance && touch.deltaTime < options.tapMaxTime) {
            if ($.options.gestureConfig.doubletap && lastTarget && (
              lastTarget === target)) { //same target
              if (lastTapTime && (
      ...
      if (options.preload) {
          ...
          if (!webview) {
            ...
            if (options.subpages) {
              $.each(options.subpages, function (index, subpage) {
                ...
                var subWebview = plus.webview.create(subpage.url, subpage.id ||
                                                                  subpage.url, $.windowOptions(subpage.styles), $.extend({
                  preload: true
                }, subpage.extras));
        ...
        if (isCreate !== false) { //直接创建非预加载窗口
          ...
          if (options.subpages) {
            $.each(options.subpages, function (index, subpage) {
              var subWebview = plus.webview.create(subpage.url, subpage.id ||
                                                                subpage.url, $.windowOptions(subpage.styles), subpage.extras);
              webview.append(subWebview);
            });
          }
        }
...
(
  function ($, document, undefined) {
    ...
    var pocketHtml = ['<div class="' + CLASS_PULL + '">', '<div class="{icon}"></div>',
                      '<div class="' + CLASS_PULL_CAPTION + '">{contentrefresh}</div>', '</div>'].join('');
    var PullRefresh = {
    ...
    var Scroll = $.Class.extend({
      ...
      _initSnap: function () {
        ...
        for (var i = 0; i < length; i++) {
          ...
          snapX = Math.round((
                               offsetWidth) * this.options.snapX);
          leftX = x - snapX;
      ...
      _getSnapX: function (offsetLeft) {
        return Math.max(Math.min(0, -offsetLeft + (
          this.wrapperWidth / 2)), this.maxScrollX);
      },
      ...
      _drag: function (e) {
        ...
        if (this.options.scrollY || detail.direction === 'up' || detail.direction === 'down') { //如果是竖向滚动或手势方向是上或下
          ...
          if ($.os.ios && parseFloat($.os.version) >= 8) { //多webview时，离开当前webview会导致后续touch事件不触发
            ...
            //下拉刷新 or 上拉加载
            if ((
                clientY + 10) > window.innerHeight || clientY < 10) {
      ...
      _flick: function (e) {
        ...
        if (this.options.momentum && detail.flickTime < 300) {
          momentumX = this.hasHorizontalScroll ?
            this._momentum(this.x, detail.flickDistanceX, detail.flickTime, this.maxScrollX, this.options.bounce ?
              this.wrapperWidth : 0, this.options.deceleration) : {
            destination: newX,
            duration: 0
          };
          momentumY = this.hasVerticalScroll ?
            this._momentum(this.y, detail.flickDistanceY, detail.flickTime, this.maxScrollY, this.options.bounce ?
      ...
      _end: function (e) {
        ...
        if ((
            !this.moved && this.needReset) || e.type === $.EVENT_CANCEL) {
          this.resetPosition();
        }
      },
      ...
      setTranslate: function (x, y) {
        ...
        if (this.parallaxElement && this.options.scrollY) { //目前仅支持竖向视差效果
          ...
          var scale = 1 + parallaxY / (
                          (
                          this.parallaxHeight - parallaxY) / 2);
          if (scale > 1) {
            this.parallaxImgStyle['opacity'] = 1 - parallaxY / 100 * this.options.parallaxRatio;
            this.parallaxStyle['webkitTransform'] = this._getTranslateStr(0, -parallaxY) + ' scale(' + scale + ',' +
                                                    scale + ')';
          } else {
    ...
    Indicator.prototype = {
      ...
      updatePosition: function () {
        ...
        if (x < this.minBoundaryX) {
          ...
          this.width = Math.max(this.indicatorWidth - (
                                x - this.maxPosX), 8);
          this.indicatorStyle.width = this.width + 'px';
        ...
        if (y < this.minBoundaryY) {
          ...
          this.height = Math.max(this.indicatorHeight - (
                                                        y - this.maxPosY) * 3, 8);
          this.indicatorStyle.height = this.height + 'px';
  ...
  function ($, window, document, undefined) {
    ...
    var PullRefresh = $.Scroll.extend($.extend({
      ...
      pulldownLoading: function (y, time) {
        typeof y === 'undefined' && (
          y = this.options.down.height); //默认高度
  ...
  function ($, window) {
    ...
    var Slider = $.Slider = $.Scroll.extend({
      ...
      _handleTabShow: function (e) {
        var self = this;
        self.gotoItem((
        e.detail.tabNumber || 0), self.options.slideTime);
      },
      ...
      _initTimer: function () {
        ...
        if (interval) {
          slidershowTimer = window.setTimeout(function () {
            ...
            //仅slider显示状态进行自动轮播
            if (!!(
              slider.offsetWidth || slider.offsetHeight)) {
              self.nextItem(true);
              //下一个
            }
            self._initTimer();
...
(
  function ($, document) {
    // 禁用h5+扽pullrefresh,全部采用div形式
    return;
    if (!(
      $.os.plus && $.os.android)) { //仅在android的5+版本使用
      return;
    }
...
(
  function ($, window, document, name) {
    ...
    var OffCanvas = $.Class.extend({
      ...
      handleEvent: function (e) {
        switch (e.type) {
          ...
          case 'drag':
            ...
            if (!this.isDragging && Math.abs(this.lastX - this.startX) > this.options.dragThresholdX && (
              ...
              if (!this.slideIn) {
                if (x >= 0) {
                  ratio = (
                          ...
                          x / this.offCanvasLeftWidth)) || 0;
                } else {
                  ratio = (
                          this.offCanvasRightWidth && (
                          x / this.offCanvasRightWidth)) || 0;
                }
                if (ratio === 0) {
                ...
                if (direction === 'right' && ratio >= 0 && (
                  ratio >= 0.5 || detail.swipe)) { //右滑打开
                  this.openPercentage(100);
                } else if (direction === 'right' && ratio < 0 && (
                  ratio > -0.5 || detail.swipe)) { //右滑关闭
                  this.openPercentage(0);
                } else if (direction === 'right' && ratio > 0 && ratio < 0.5) { //右滑还原关闭
                  this.openPercentage(0);
                } else if (direction === 'right' && ratio < 0.5) { //右滑还原打开
                  this.openPercentage(-100);
                } else if (direction === 'left' && ratio <= 0 && (
                  ratio <= -0.5 || detail.swipe)) { //左滑打开
                  this.openPercentage(-100);
                } else if (direction === 'left' && ratio > 0 && (
                  ratio <= 0.5 || detail.swipe)) { //左滑关闭
                  this.openPercentage(0);
      ...
      setTranslateX: $.animationFrame(function (x) {
        if (this.scroller) {
          if (this.scalable && this.offCanvas.parentNode === this.wrapper) {
            ...
            var zoomOutScale = 1 - (
                                   1 - this.options.scale) * percent;
            var zoomInScale = this.options.scale + (
                                                   1 - this.options.scale) * percent;
            var zoomOutOpacity = 1 - (
                                     1 - this.options.opacity) * percent;
            var zoomInOpacity = this.options.opacity + (
                                                       1 - this.options.opacity) * percent;
      ...
      close: function () {
        this._initOffCanvasVisible();
        this.offCanvas = this.wrapper.querySelector('.' + CLASS_OFF_CANVAS_RIGHT + '.' + CLASS_ACTIVE) ||
                         this.wrapper.querySelector('.' + CLASS_OFF_CANVAS_LEFT + '.' + CLASS_ACTIVE);
...
(
  function ($, window, document, name) {
    ...
    var calPosition = function (popover, anchor, isActionSheet) {
      ...
      if ((
          pHeight + arrowSize) < (
          offset.top - window.pageYOffset)) { //top
        pTop = offset.top - pHeight - arrowSize;
      } else if ((
                 pHeight + arrowSize) < (
                 wHeight - (
                 offset.top - window.pageYOffset) - aHeight)) { //bottom
        position = 'bottom';
        pTop = offset.top + aHeight + arrowSize;
      } else { //middle
        position = 'middle';
        pTop = Math.max((
                        wHeight - pHeight) / 2 + window.pageYOffset, 0);
        pLeft = Math.max((
                         wWidth - pWidth) / 2 + window.pageXOffset, 0);
      }
...
(
  function ($, window, document, name, undefined) {
    ...
    var handle = function (event, target) {
      /**
       * 取消mui对底部导航点击hashChange的阻止，否则vue-router的v-link在移动端无法使用。*/
  ...
  function ($, window, name) {
    ...
    Toggle.prototype.setTranslateX = $.animationFrame(function (x) {
      ...
      if (this.lastChanged !== isChanged) {
        if (isChanged) {
          this.handle.style.webkitTransform = 'translate(' + (
              this.initialState ? 0 : this.handleX) + 'px,0)';
          this.classList[this.initialState ? 'remove' : 'add'](CLASS_ACTIVE);
        } else {
          this.handle.style.webkitTransform = 'translate(' + (
              this.initialState ? this.handleX : 0) + 'px,0)';
...
(
  function ($, window, document) {
      ...
      for (; target && target !== document; target = target.parentNode) {
        if (target.classList) {
          ...
          if ((
              ...
              target.tagName === 'BUTTON' || classList.contains(CLASS_TOGGLE) || classList.contains(CLASS_BTN) ||
    ...
    var handleEvent = {
      ...
      drag: function (event) {
        ...
        if (direction === 'left' && (
          angle > 150 || angle < -150)) {
          if (buttonsRight || (
            buttonsLeft && isOpened)) { //存在右侧按钮或存在左侧按钮且是已打开状态
            isMoved = true;
          }
        } else if (direction === 'right' && (
                   angle > -30 && angle < 30)) {
          if (buttonsLeft || (
            buttonsRight && isOpened)) { //存在左侧按钮或存在右侧按钮且是已打开状态
            isMoved = true;
          }
        }
        if (isMoved) {
      ...
      dragend: function (event) {
        ...
        var isToggle = detail.swipe || (
          Math.abs(translateX) > actionsWidth / 2);
        if (isToggle) {
          if (!isOpened) {
            action = 'open';
    ...
    $.swipeoutOpen = function (el, direction) {
      ...
      var translate = (
      direction === 'right') ? -swipeoutWidth : swipeoutWidth;
  ...
  function ($, window, document) {
    ...
    var createInner = function (message, title, extra) {
      return '<div class="' + CLASS_POPUP_INNER + '"><div class="' + CLASS_POPUP_TITLE + '">' + title +
             '</div><div class="' + CLASS_POPUP_TEXT + '">' + message + '</div>' + (
             extra || '') + '</div>';
    };
    var createButtons = function (btnArray) {
      ...
      var btns = [];
      for (var i = 0; i < length; i++) {
        btns.push('<span class="' + CLASS_POPUP_BUTTON + (
            i === length - 1 ? (
    ...
    var createPrompt = function (message, placeholder, title, btnArray, callback, type) {
      ...
      if (!$.os.plus || type === 'div') {
        return createPopup(createInner(message, title || '提示', createInput(placeholder)) +
  ...
  function ($, window, document) {
    ...
    Picker.prototype.calcElementItemPostion = function (andGenerateItms) {
      ...
      self.elementItems.forEach(function (item) {
        ...
        item.style.webkitTransform = "translateZ(" + self.r + "px) rotateX(" + (
            -self.endAngle) + "deg)";
        if (andGenerateItms) {
          var dataItem = {};
    ...
    Picker.prototype.startInertiaScroll = function (event) {
      ...
      var nowTime = event.timeStamp || Date.now();
      var v = (
              point.pageY - self.lastMoveStart) / (
              nowTime - self.lastMoveTime); //最后一段时间手指划动速度
    ...
    Picker.prototype.endScroll = function () {
      ...
      if (self.list.angle < self.beginAngle) {
        ...
        var index = parseInt((
        self.list.angle / self.itemAngle).toFixed(0));
    ...
    Picker.prototype.getSelectedValue = function () {
      var self = this;
      return (
      self.items[self.getSelectedIndex()] || {}).value;
    };
    Picker.prototype.getSelectedText = function () {
      var self = this;
      return (
      self.items[self.getSelectedIndex()] || {}).text;
    };
...
(
  function ($, document) {
    ...
    var panelBuffer = '<div class="mui-poppicker">\
		<div class="mui-poppicker-header" style="position: relative">\
			<button class="mui-btn mui-poppicker-btn-cancel">取消</button>\
			<button class="mui-btn mui-btn-success reset" style="position: absolute; left: 50%; margin-left: -22px">重置\
						</button>\
    ...
    var PopPicker = $.PopPicker = $.Class.extend({
      ...
      init: function (options) {
        ...
        self.reset = self.panel.querySelector('.reset');
        self.body = self.panel.querySelector('.mui-poppicker-body');
        self.mask = $.createMask();
        self.cancel.innerText = self.options.buttons[0];
        self.ok.innerText = self.options.buttons[1];
        // 控制是否显示重置按钮
        if(!self.options.resetShow) {
        ...
        self.reset.addEventListener('tap', function (event) {
          // alert('你按了重置按钮');
          self.options.reset();
          self.hide();
        }, false);
...
(
  function ($, window) {
    ...
    proto.init = function () {
      ...
      if (!el) {
        div.innerHTML = template.replace(/\{\{id\}\}/g, this.options.id)
    ...
    proto.initEvent = function () {
      ...
      $(document.body).on('tap', 'img[data-preview-src]', function () {
        // console.log('打开预览时的Index' + this.getAttribute('data-index'));
        // 打开预览时候的函数
        if (options.openfn) {
          options.openfn(this.getAttribute('data-index'));
        }
        // 打开预览时候的函数end
        self.open(this);
        return false;
      ...
      this.element.addEventListener('slide', function (e) {
        if (self.options.zoom) {
          var lastZoomerEl = self.element.querySelector('.mui-zoom-wrapper:nth-child(' + (
                                                        self.lastIndex + 1) + ')');
          if (lastZoomerEl) {
        ...
        self.indicator && (
          self.indicator.innerText = (
                                     slideNumber + 1) + '/' + self.currentGroup.length);
        self._loadItem(slideNumber);
        // 滑动函数的调用
        if (options.slidefn) {
          options.slidefn(slideNumber);
        }
        // 滑动函数的调用 end
      });
    };
    ...
    proto._loadItem = function (index, isOpening) { //TODO 暂时仅支持img
      var itemEl = this.scroller.querySelector($.classSelector('.slider-item:nth-child(' + (
                                                               index + 1) + ')'));
    ...
    proto._getPosition = function (itemData) {
      ...
      var left = (
                 window.innerWidth - itemData.sWidth) / 2;
      var top = (
                window.innerHeight - itemData.sHeight) / 2;
      return {
        left: sLeft,
        top: sTop,
        x: sLeft - left,
    ...
    proto.refresh = function (index, groupArray) {
      ...
      for (var i = 0; from < to; from++, i++) {
        ...
        var style = '';
        if (itemData.sWidth) {
          style = '-webkit-transform:translate3d(0,0,0) scale(' + itemData.sScale +
                  ');transform:translate3d(0,0,0) scale(' + itemData.sScale + ')';
        }
        itemStr = itemTemplate.replace('{{src}}', itemData.src)
      ...
      this.indicator && (
        this.indicator.innerText = (
                                   currentIndex + 1) + '/' + this.currentGroup.length);
      this._loadItem(currentIndex, true);
      if (this.options.longTapfn) {
        setTimeout(function() {
          document.querySelectorAll('#' + self.options.id + ' img').forEach(function (item) {
            item.addEventListener('longtap', self.options.longTapfn);
          });
        }, 0);
      }
    };
  ...
  function ($, window) {
    ...
    $.Zoom = function (element, options) {
      ...
      zoom.onPinch = function (e) {
        ...
        scale = (
                  e.detail ? e.detail.scale : e.scale) * currentScale;
        if (scale > zoom.options.maxZoom) {
          scale = zoom.options.maxZoom - 1 + Math.pow((
                  scale - zoom.options.maxZoom + 1), 0.5);
        }
        if (scale < zoom.options.minZoom) {
          scale = zoom.options.minZoom + 1 - Math.pow((
            zoom.options.minZoom - scale + 1), 0.5);
        }
      ...
      zoom.toggleZoom = function (position, speed) {
        ...
        if (scale && scale !== 1) {
          ...
          if (position) {
            ...
            var offsetX = (
                          position.x - left) * scale;
            var offsetY = (
                          position.y - top) * scale;
            this._cal();
            if (offsetX >= imageMaxX && offsetX <= (
              imageMaxX + wrapperWidth)) { //center
              offsetX = imageMaxX - offsetX + wrapperWidth / 2;
            } else if (offsetX < imageMaxX) { //left
              offsetX = imageMaxX - offsetX + wrapperWidth / 2;
            } else if (offsetX > (
              imageMaxX + wrapperWidth)) { //right
              offsetX = imageMaxX + wrapperWidth - offsetX - wrapperWidth / 2;
            }
            if (offsetY >= imageMaxY && offsetY <= (
              imageMaxY + wrapperHeight)) { //middle
              offsetY = imageMaxY - offsetY + wrapperHeight / 2;
            } else if (offsetY < imageMaxY) { //top
              offsetY = imageMaxY - offsetY + wrapperHeight / 2;
            } else if (offsetY > (
              imageMaxY + wrapperHeight)) { //bottom
      ...
      zoom._cal = function () {
        ...
        imageMinX = Math.min((
        wrapperWidth / 2 - scaledWidth / 2), 0);
        imageMaxX = -imageMinX;
        imageMinY = Math.min((
        wrapperHeight / 2 - scaledHeight / 2), 0);
        imageMaxY = -imageMinY;
      };
      ...
      zoom.onTouchMove = function (e) {
        ...
        if (imageCurrentX < imageMinX) {
          imageCurrentX = imageMinX + 1 - Math.pow((
            imageMinX - imageCurrentX + 1), 0.8);
        }
        if (imageCurrentX > imageMaxX) {
          imageCurrentX = imageMaxX - 1 + Math.pow((
                          imageCurrentX - imageMaxX + 1), 0.8);
        }
        if (imageCurrentY < imageMinY) {
          imageCurrentY = imageMinY + 1 - Math.pow((
            imageMinY - imageCurrentY + 1), 0.8);
        }
        if (imageCurrentY > imageMaxY) {
          imageCurrentY = imageMaxY - 1 + Math.pow((
                          imageCurrentY - imageMaxY + 1), 0.8);
        }
        //Velocity
  ...
  function ($, window, document) {
    ...
    var IndexedList = $.IndexedList = $.Class.extend({
      ...
      findElements: function () {
        ...
        self.el.searchClear = self.box.querySelector(classSelector('indexed-list-search') + ' ' +
      ...
      search: function (keyword) {
        ...
        var checkGroup = function (currentIndex, last) {
          if (itemCount >= currentIndex - groupIndex - (
              last ? 0 : 1)) {
            selectorBuffer.push(classSelector('indexed-list-inner li') + ':nth-child(' + (
                                groupIndex + 1) + ')');
